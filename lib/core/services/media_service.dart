...
class MediaService {
  Future<String?> uploadAvatar(String userId, XFile imageFile) async {
    try {
      AppLogger.action(LogCategory.system, 'AVATAR_UPLOAD_START',
        {'userId': userId, 'originalSize': await imageFile.length()});
      ...
      AppLogger.error(LogCategory.system, 'AVATAR_UPLOAD_FAILED', error: e, stack: st);
      return null;
    }
  }
  ...
  Future<String?> uploadProjectImage(String projectId, XFile imageFile) async {
    try {
      ...
      AppLogger.error(LogCategory.system, 'PROJECT_IMAGE_FAILED', error: e, stack: st);
      return null;
    }
  }

  Future<Uint8List> _compress(XFile file,
      {required int quality, required int maxDim}) async {
