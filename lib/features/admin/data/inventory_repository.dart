...
class InventoryRepository {
  ...
  Future<List<InventoryItemModel>> getInventoryItems({String? category, String? type}) async {
    AppLogger.action('Inventory', 'getInventoryItems', data: {'category': category, 'type': type});
    try {
      ...
      AppLogger.error('Inventory', 'getInventoryItems failed', e, st);
      rethrow;
    }
  }
  ...
  Future<void> adjustStock({
    ...
    AppLogger.action('Inventory', 'adjustStock', data: {'itemId': itemId, 'change': change});
    try {
      ...
      AppLogger.info('Inventory', 'Stock adjusted for $itemId: $change');
    } catch (e, st) {
      AppLogger.error('Inventory', 'adjustStock failed', e, st);
      rethrow;
    }
  }
  ...
  Future<void> logToolStatus({
    ...
    AppLogger.action('Inventory', 'logToolStatus', data: {'toolId': toolId, 'type': transactionType});
    try {
      ...
      AppLogger.info('Inventory', 'Tool $toolId status updated to $newStatus');
    } catch (e, st) {
      AppLogger.error('Inventory', 'logToolStatus failed', e, st);
      rethrow;
    }
  }
  ...
  Future<List<InventoryTransactionModel>> getTransactionHistory({String? toolId, String? itemId}) async {
    try {
      ...
      AppLogger.error('Inventory', 'getTransactionHistory failed', e, st);
      rethrow;
    }
  }
}
