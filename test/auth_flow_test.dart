...
void main() {
  group('AuthFlow Unit & Integration Tests', () {
    ...
    late MockPostgrestTransformBuilder<Map<String, dynamic>?> mockTransformBuilder;
    ...
    setUp(() {
      ...
      mockTransformBuilder = MockPostgrestTransformBuilder<Map<String, dynamic>?>();
    ...
    test('Sign-in flow executes successfully', () async {
      ...
      when(() => mockFilterBuilder.eq('id', 'user-123')).thenAnswer((_) => mockFilterBuilder);
      ...
      when(() => mockTransformBuilder.timeout(any())).thenAnswer((_) async => {'id': 'user-123'});
    ...
    test('Sign-up flow creates auth user and public database entry', () async {
      ...
      final mockInsertFilterBuilder = MockPostgrestFilterBuilder<List<Map<String, dynamic>>>();
      when(() => mockQueryBuilder.insert(any())).thenAnswer((_) => mockInsertFilterBuilder);
      
      // Stub the timeout call on the filter builder to return the list
      when(() => mockInsertFilterBuilder.timeout(any())).thenAnswer((_) async => <Map<String, dynamic>>[]);
